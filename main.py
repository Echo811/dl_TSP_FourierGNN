...
# data set
data_parser = {
    'traffic':{'root_path':'data/traffic.csv', 'type':'0'},
...
print(f'train: {train_dataloader.dataset.__len__()}')
print(f'test: {test_dataloader.dataset.__len__()}')
print(f'val: {val_dataloader.dataset.__len__()}')
...
def validate(model, vali_loader):
    ...
    # 序列化对象
    save_({'pre_val': preds, 'true_val': trues})
...
def test():
    ...
    trues = np.concatenate(trues, axis=0)
    # 序列化对象
    save_({'pre_test':preds, 'true_test':trues})
    score = evaluate(trues, preds)
    print(f'RAW : MAPE {score[0]:7.9%}; MAE {score[1]:7.9f}; RMSE {score[2]:7.9f}.')
